...
for ($i = 0; $i < count($_GET); $i++) {
	$SubQuery = mysqli_query($db_handle, "SELECT * FROM users WHERE username='".$_GET["user".$i]."';");
	if ($queryData = mysqli_fetch_assoc($SubQuery)) {
		$subs = $queryData['ThreadsSubscribed'];
		$subs = $_GET['filename']."`".$subs;
		$updateQuery = mysqli_query($db_handle, "UPDATE users SET ThreadsSubscribed='".$subs."' WHERE username='".$_GET["user".$i]."';");
		$output .= $_GET['user'.$i]."`";
		
		$contribs .= $_GET['user'.$i]."`";
	}
}
